...
class ReviewController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $reviews = Review::with('lieuDepot')->get(); // Fetch reviews with related lieuDepot data
    
            return DataTables::of($reviews)
                ->addIndexColumn()
                ->setRowClass(fn ($row) => 'align-middle')
                ->addColumn('lieu_depot', function ($review) {
                    return $review->lieuDepot ? $review->lieuDepot->nom : 'N/A'; // Ensure you access lieuDepot correctly
                })
